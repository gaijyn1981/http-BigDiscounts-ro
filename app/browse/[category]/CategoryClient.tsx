      ...
      <div className="px-6 py-8" style={{ background: '#111111', borderBottom: '1px solid #1a1a1a' }}>
        <div className="max-w-5xl mx-auto">
          <p className="text-sm mb-2" style={{ color: '#fcd968' }}>
            <Link href="/browse" className="hover:underline">Răsfoiește</Link> › {category}
          </p>
          ...
          <div className="flex gap-3 flex-wrap">
            <input type="text" placeholder="Caută în această categorie..." value={search} onChange={e => setSearch(e.target.value)}
          ...
          <div className="flex gap-3 mt-3 items-center">
            ...
            <input type="number" placeholder="Max £" value={maxPrice} onChange={e => setMaxPrice(e.target.value)}
              ...
              style={{ background: '#1a1a1a', border: '1px solid #333' }} />
            {(minPrice || maxPrice) && (
              <button onClick={() => { setMinPrice(''); setMaxPrice('') }} className="text-sm text-red-400 hover:text-red-300">Șterge</button>
            )}
          </div>
        </div>
      </div>
      ...
      <div className="max-w-6xl mx-auto px-6 py-10">
        {loading ? (
          ...
          <div className="text-center py-20">
            ...
            <p className="text-gray-400 text-lg font-bold">Nu există anunțuri în această categorie încă.</p>
            ...
            <Link href="/browse"
              ...
              Răsfoiește toate categoriile
            </Link>
          </div>
        ) : (
          <>
            <p className="text-gray-600 text-sm mb-6">{filtered.length} produs{filtered.length !== 1 ? 'e' : ''} găsit{filtered.length !== 1 ? 'e' : ''} în {category}</p>
